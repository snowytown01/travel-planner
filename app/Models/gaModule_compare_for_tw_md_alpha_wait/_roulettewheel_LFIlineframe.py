...
class GeneticAlgo:
    ...
    def evolvepopulation(self, oldpopulation):
        ...
        ###########수정##############
        # for q in newpopulation.tours:
        #     for p in q.tour:
        #         print(p.getname(), ':', p.getalpha(), end='/')    #테스트

        #     #     print("{}({}): {}".format(p.getname(), p.getutil(), p.getalpha()), end='   ')
        #     # print('tour_fitness:', q.getfitness())
        #     # print('')
    ...
    def crossover(self, parent1, parent2):      # parent1, parent2, child are all "tour"s
        ...
        routefitnessdict = {}
        for portionstart in range(0, parent1.toursize()//2 + 1):
            historynode = []
        ...
        for ele in child.tour:
            if ele in tempp2nodes:
                tempp2nodes.remove(ele)

        while len(tempp2nodes) != 0:
            ele = tempp2nodes.pop(0)
            record = {}
